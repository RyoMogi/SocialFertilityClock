...
Getlx <- function(cntr, MinAge, BC, union){
  ...
  # Exposure
  bc <- c("1930s", "1940s", "1950s", "1960s", "1970s")
  ...
  for(j in 1:length(bc)){
    ...
    for(i in 1:length(status)){
      ...
      for(k in 1:nrow(D) - 1){
        exp[k] <- (D$value[k] + D$value[k+1]) / 2
      }
      
      expo <- cbind(expo, exp)
    }
    
    colnames(expo) <- c("S1P0", "S1P1", "S1P2", "C1P0", "C1P1", "C1P2", "M1P0", "M1P1", "M1P2")
  ...
  for(i in 1:length(bc)){
    ...
    S1P_event[[i]] <- table(D$S1Pevent, D$age)
    ...
    C1P_event[[i]] <- table(D$C1Pevent, D$age)
    ...
    M1P_event[[i]] <- table(D$M1Pevent, D$age)
    
    event[[i]] <- rbind(S1P_event[[i]], C1P_event[[i]], M1P_event[[i]])
  }
  ...
  mij <- function(i, j){ # i: original state, j: transited state
    ...
    for(x in 1:length(bc)){
      alltransit <- rownames(event[[x]])
      
      if(j == "S1P0"){
        out <- exposure[[x]][, j] / exposure[[x]][, i]
        
      } else {
        
        if(transit %in% alltransit){
          out <- event[[x]][transit, -36] / exposure[[x]][, i]
    ...
    colnames(mij) <- c("bc1930s", "bc1940s", "bc1950s", "bc1960s", "bc1970s")
  ...
  matrix_Mij <- function(bc){
    mat_Mij <- list()
    Mij <- matrix(NA, nrow = 9, ncol = 9)
    
    for(age in 1:nrow(m_S1P0C1P0)){
      Mij[1, 1] <- m_S1P0C1P0[age, bc] + m_S1P0M1P0[age, bc] + m_S1P0S1P1[age, bc] + m_S1P0C1P1[age, bc] + m_S1P0M1P1[age, bc]
      Mij[1, 2] <- - m_S1P0C1P0[age, bc]
      Mij[1, 3] <- - m_S1P0M1P0[age, bc]
      Mij[1, 4] <- - m_S1P0S1P1[age, bc]
      Mij[1, 5] <- - m_S1P0C1P1[age, bc]
      Mij[1, 6] <- - m_S1P0M1P1[age, bc]
      Mij[1, 7] <- 0
      Mij[1, 8] <- 0
      Mij[1, 9] <- 0
      Mij[2, 1] <- 0
      Mij[2, 2] <- m_C1P0M1P0[age, bc] + m_C1P0C1P1[age, bc] + m_C1P0M1P1[age, bc]
      Mij[2, 3] <- - m_C1P0M1P0[age, bc]
      Mij[2, 4] <- 0
      Mij[2, 5] <- - m_C1P0C1P1[age, bc]
      Mij[2, 6] <- - m_C1P0M1P1[age, bc]
      Mij[2, 7] <- 0
      Mij[2, 8] <- 0
      Mij[2, 9] <- 0
      Mij[3, 1] <- 0
      Mij[3, 2] <- 0
      Mij[3, 3] <- m_M1P0M1P1[age, bc]
      Mij[3, 4] <- 0
      Mij[3, 5] <- 0
      Mij[3, 6] <- - m_M1P0M1P1[age, bc]
      Mij[3, 7] <- 0
      Mij[3, 8] <- 0
      Mij[3, 9] <- 0
      Mij[4, 1] <- 0
      Mij[4, 2] <- 0
      Mij[4, 3] <- 0
      Mij[4, 4] <- m_S1P1C1P1[age, bc] + m_S1P1M1P1[age, bc] + m_S1P1S1P2[age, bc] + m_S1P1C1P2[age, bc] + m_S1P1M1P2[age, bc]
      Mij[4, 5] <- - m_S1P1C1P1[age, bc]
      Mij[4, 6] <- - m_S1P1M1P1[age, bc]
      Mij[4, 7] <- - m_S1P1S1P2[age, bc]
      Mij[4, 8] <- - m_S1P1C1P2[age, bc]
      Mij[4, 9] <- - m_S1P1M1P2[age, bc]
      Mij[5, 1] <- 0
      Mij[5, 2] <- 0
      Mij[5, 3] <- 0
      Mij[5, 4] <- 0
      Mij[5, 5] <- m_C1P1M1P1[age, bc] + m_C1P1C1P2[age, bc] + m_C1P1M1P2[age, bc]
      Mij[5, 6] <- - m_C1P1M1P1[age, bc]
      Mij[5, 7] <- 0
      Mij[5, 8] <- - m_C1P1C1P2[age, bc]
      Mij[5, 9] <- - m_C1P1M1P2[age, bc]
      Mij[6, 1] <- 0
      Mij[6, 2] <- 0
      Mij[6, 3] <- 0
      Mij[6, 4] <- 0
      Mij[6, 5] <- 0
      Mij[6, 6] <- m_M1P1M1P2[age, bc]
      Mij[6, 7] <- 0
      Mij[6, 8] <- 0
      Mij[6, 9] <- - m_M1P1M1P2[age, bc]
      Mij[7, 1] <- 0
      Mij[7, 2] <- 0
      Mij[7, 3] <- 0
      Mij[7, 4] <- 0
      Mij[7, 5] <- 0
      Mij[7, 6] <- 0
      Mij[7, 7] <- m_S1P2C1P2[age, bc] + m_S1P2M1P2[age, bc]
      Mij[7, 8] <- - m_S1P2C1P2[age, bc]
      Mij[7, 9] <- - m_S1P2M1P2[age, bc]
      Mij[8, 1] <- 0
      Mij[8, 2] <- 0
      Mij[8, 3] <- 0
      Mij[8, 4] <- 0
      ...
      Mij[8, 6] <- 0
      Mij[8, 7] <- 0
      Mij[8, 8] <- m_C1P2M1P2[age, bc]
      Mij[8, 9] <- - m_C1P2M1P2[age, bc]
      Mij[9, 1] <- 0
      Mij[9, 2] <- 0
      Mij[9, 3] <- 0
      Mij[9, 4] <- 0
...
server <- function(input, output) {
  ...
  sel_lx_S_1940s <- reactive({
    SWE_15_1940s <- Getlx(cntr = cntr(), MinAge = input$MinAge, BC = "bc1940s", union = "S")
    SWE_15_1940s <- ifelse(SWE_15_1940s < 0, 0, SWE_15_1940s)
  })
  
  sel_lx_S_1950s <- reactive({
    SWE_15_1950s <- Getlx(cntr = cntr(), MinAge = input$MinAge, BC = "bc1950s", union = "S")
    SWE_15_1950s <- ifelse(SWE_15_1950s < 0, 0, SWE_15_1950s)
  })
  
  sel_lx_S_1960s <- reactive({
    SWE_15_1960s <- Getlx(cntr = cntr(), MinAge = input$MinAge, BC = "bc1960s", union = "S")
    SWE_15_1960s <- ifelse(SWE_15_1960s < 0, 0, SWE_15_1960s)
  })
  
  ## cohabitation
  sel_lx_C_1940s <- reactive({
    SWE_15_1940s <- Getlx(cntr = cntr(), MinAge = input$MinAge, BC = "bc1940s", union = "C")
    SWE_15_1940s <- ifelse(SWE_15_1940s < 0, 0, SWE_15_1940s)
  })
  
  sel_lx_C_1950s <- reactive({
    SWE_15_1950s <- Getlx(cntr = cntr(), MinAge = input$MinAge, BC = "bc1950s", union = "C")
    SWE_15_1950s <- ifelse(SWE_15_1950s < 0, 0, SWE_15_1950s)
  })
  
  sel_lx_C_1960s <- reactive({
    SWE_15_1960s <- Getlx(cntr = cntr(), MinAge = input$MinAge, BC = "bc1960s", union = "C")
    SWE_15_1960s <- ifelse(SWE_15_1960s < 0, 0, SWE_15_1960s)
  })
  
  ## married
  sel_lx_M_1940s <- reactive({
    SWE_15_1940s <- Getlx(cntr = cntr(), MinAge = input$MinAge, BC = "bc1940s", union = "M")
    SWE_15_1940s <- ifelse(SWE_15_1940s < 0, 0, SWE_15_1940s)
  })
  
  sel_lx_M_1950s <- reactive({
    SWE_15_1950s <- Getlx(cntr = cntr(), MinAge = input$MinAge, BC = "bc1950s", union = "M")
    SWE_15_1950s <- ifelse(SWE_15_1950s < 0, 0, SWE_15_1950s)
  })
  
  sel_lx_M_1960s <- reactive({
    SWE_15_1960s <- Getlx(cntr = cntr(), MinAge = input$MinAge, BC = "bc1960s", union = "M")
    SWE_15_1960s <- ifelse(SWE_15_1960s < 0, 0, SWE_15_1960s)
  })
